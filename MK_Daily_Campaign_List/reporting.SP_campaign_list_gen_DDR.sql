DROP PROCEDURE IF EXISTS reporting.SP_campaign_list_gen_DDR;
CREATE PROCEDURE reporting.`SP_campaign_list_gen_DDR`()
BEGIN
/******************************************************************************************************************
---    NAME : SP_campaign_list_gen_DDR
...
---    04/07/20017                   DAT-143 DDR list holiday roll over issue
---    05/09/2017                    DAT-202 DDR campaign list generate on holiday with duplicate record issue
----   20/09/2017                    DAT-212 add additional columns for ddr5
...
********************************************************************************************************************/
  ...
  SET @process_name = 'SP_campaign_list_gen_DDR', @status_flag_success = 1, @status_flag_failure = 0;
  ...
  select count(*) into NotRunFlag
	...
	IF NotRunFlag = 0 THEN
      ...
      @valuation_date, 
			...
			(case when la.state in ('TN', 'KS') then date_format(@list_gen_time, 'ALOC%m%d%YDDR') else date_format(@list_gen_time, 'JAG%m%d%YDDR') end) as job_ID,
      ...
      @valuation_date,
			...
			CONCAT(UCASE(SUBSTRING(la.customer_lastname, 1, 1)),LOWER(SUBSTRING(la.customer_lastname, 2))) as Customer_LastName,
      la.origination_time as `origination_time`, -- DAT-647
			if(pr.IsAfterHoliday=1,(select adm.post_target_date from reporting.vw_DDR_ach_date_matching adm where date(p.DateOfTotalDue)=adm.ori_date), 
      (select adm.pre_target_date from reporting.vw_DDR_ach_date_matching adm where date(p.DateOfTotalDue)=adm.ori_date)) as ach_date,
      ...
      la.lms_application_id as origination_loan_id
			...
			where DATE(if(pr.IsAfterHoliday=1,(select adm.post_target_date from reporting.vw_DDR_ach_date_matching adm where date(p.DateOfTotalDue)=adm.ori_date), 
      (select adm.pre_target_date from reporting.vw_DDR_ach_date_matching adm where date(p.DateOfTotalDue)=adm.ori_date))) in (@first_date, @second_date)
			and la.State != 'VA'
    ...
    BEGIN
    	...
    	DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
			...
			INSERT INTO reporting.campaign_history
                  ...
                  @channel,
                  @list_name as list_name,
                  (case when la.state in ('TN', 'KS') then date_format(@list_gen_time, 'ALOC%m%d%YDDR5') else date_format(@list_gen_time, 'JAG%m%d%YDDR5') end) as job_ID,
    ...
    BEGIN
     ...
     -- move thw following jobs to hourly run
          ...
          where holiday = NextBusinessDate 
            and description in( 'Thanksgiving Day', 'Christmas', 'New Year''s Day');
				IF Is_predefined_no_run_day = 0 then 
					...
					CALL reporting.SP_campaign_list_gen_DDR_holiday(NextBusinessDate);
			 END IF;
		 END IF;
  END IF;
