    ...
    @valuation_date =curdate();
	 ...
	 set
      ...
      and la.state in ('DE','IL','KS','MO','NM','TX','UT','AL', 'MS','ID','WI') -- exclude TN
 ...
 reporting.campaign_history(business_date, Channel, list_name, job_ID, list_module,
  list_frq, lms_customer_id, lms_application_id, received_time, lms_code, state, product,
   ...
   extra1, key_word)
      ...
      when WEEKDAY(@valuation_date)=1 then t2.state='NM'
      when WEEKDAY(@valuation_date)=2 then t2.state in ('KS', 'MS', 'IL')
      when WEEKDAY(@valuation_date)=3 then t2.state in ('ID','UT','DE','AL')
      end;
 ...
 reporting.campaign_history(business_date, Channel, list_name, job_ID, list_module,
  list_frq, lms_customer_id, lms_application_id, received_time, lms_code, state, product,
   ...
   extra1,key_word)
      ...
      t2.group_filer
      from table3 t2
      where t2.group_filer='Group2' and t2.IsValidEmail=1
       and 
      case 
      when WEEKDAY(@valuation_date)=0 then t2.state in('DE','IL','WI','MS','AL','ID')
      when WEEKDAY(@valuation_date)=1 then t2.state='MO'
      when WEEKDAY(@valuation_date)=2 then t2.state='KS'
      when WEEKDAY(@valuation_date)=3 then t2.state in('UT','NM')
      end ;
  ...
  list_frq, lms_customer_id, lms_application_id, received_time, lms_code, state, product,
   ...
   and email not in (select email_address from temp.PRJ_Bounce_list);
   
   
   
      ##Group2: Jasper report query   
      select 
      business_date, Channel, list_name, job_ID, list_module,
  list_frq, lms_customer_id, lms_application_id, received_time, lms_code, state, product,
   case
   ...
   extra1 as days_since_last_withdraw,key_word as Group_Filter
   from reporting.campaign_history
   where list_module='WA-PRJ' and business_date=curdate() and key_word='Group2'
      and email not in (select email_address from temp.PRJ_Bounce_list);
