DROP PROCEDURE IF EXISTS reporting.SP_campaign_list_gen_W_WA;
CREATE PROCEDURE reporting.`SP_campaign_list_gen_W_WA`()
BEGIN
/***************************************************************************************************************************************
---    NAME : SP_campaign_list_gen_W_WA
...
---    30/05/2017    Eric Pu         DAT-71 - change the SP running schedule based on holiday table
--- 																 will run on every Tuesday except New Year (Jan 1), Christmas (Dec 25) and US Thanksgiving Day
---    02/06/2017    Eric Pu         DAT-77  exclude customers in MD since company is pausing the service in MaryLand 
---    07/06/2017    Eric Pu         DAT-72 add business_date column
---    19/06/2017    Eric Pu         DAT-123 change SP to use the IsApplicationTest flag in leads_accepted, also add tester email for the 
---                                  email process verification               
---    20/09/2017                    DAT-212 Add ‘AL’ to all campaign list      
---    06/11/2017                    DAT-253 Update reporting.SP_campaign_list_gen_W_WA for the logic to handle withdrawn DDR  
---    17/09/2018                    DAT-579 Add Withdrawn_time to reporting.SP_campaign_list_gen_W_WA
****************************************************************************************************************************************/
  ...
  SET @process_name = 'SP_campaign_list_gen_W_WA', @status_flag_success = 1, @status_flag_failure = 0;
 ...
 SELECT count(*) INTO IsHoliday
  ...
  END IF;
    ...
    @test_job_id = 'JAG_TEST_WAJ'; 
		...
		(select b.Ori_Date from reporting.vw_DDR_ach_date_matching a join reporting.vw_DDR_ach_date_matching b on b.weeknum=(a.weeknum-@std_week_interval) and b.weekdate=3 and a.Ori_Date=date(@list_gen_time) and year(b.ori_date)=year(a.Ori_Date))),
		@end_date= if(weekday(@list_gen_time) in (5,6),0,
		(select b.Ori_Date from reporting.vw_DDR_ach_date_matching  a join reporting.vw_DDR_ach_date_matching b on b.weeknum=(a.weeknum-@end_week_interval) and b.weekdate=2 and a.Ori_Date=date(@list_gen_time) and year(b.ori_date)=year(a.Ori_Date))),
		@comment='Withdrawn Application received during -1 W Tue day to Mon';
      ...
      la.withdrawn_time, -- DAT-579
    	...
    	DECLARE CONTINUE HANDLER FOR SQLEXCEPTION
			...
			Customer_LastName,      Req_Loan_Amount,Max_Loan_Limit,withdrawn_reason, withdrawn_time, key_word,list_generation_time, extra1)
