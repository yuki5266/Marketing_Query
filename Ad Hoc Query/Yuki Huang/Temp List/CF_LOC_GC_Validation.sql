...
DROP TEMPORARY TABLE IF EXISTS tmp_all_customer;
      ...
      -- select * from tmp_all_customer2;
      -- select * from reporting_cf.vcf_lms_payment_schedules where customer_id=1004452;                                                
			...
			SET @process_label ='CF loc_gc_campaign_history - main process', @process_type = 'Insert';
      ...
      -- missing 'total_payment_count' on the select statement
            ...
            select * from reporting.ETL_process_log where process_name = 'SP_loc_gc_campaign_history_CF';
###########################################################            
            
            select approved_amount from reporting_cf.leads_accepted where lms_customer_id='1048364';
            
            
            select * from jaglms.lms_payment_schedule_items where payment_schedule_id in
(select payment_schedule_id from jaglms.lms_payment_schedules where base_loan_id =1106710);


select * from payment_schedule_id jaglms.lms_payment_schedules

select la.lms_code, la.lms_customer_id, la.lms_application_id, 
la.loan_number, la.state, la.product, la.loan_status, la.isoriginated,
la.received_time, la.loan_sequence, la.application_status
from reporting_cf.leads_accepted la WHERE  la.lms_code='JAG' 
			-- and la.isoriginated=1
			and la.origination_time>'2019-06-10'
			and la.product='LOC'
			-- and la.loan_status ='Originated'
			and la.isapplicationtest=0;


select * from reporting_cf.leads_accepted where lms_customer_id=1086438 and isapplicationtest=0;


select * from reporting_cf.leads_accepted where loan_sequence>1;

select *from reporting_cf.leads_accepted where loan_status='REJECT';