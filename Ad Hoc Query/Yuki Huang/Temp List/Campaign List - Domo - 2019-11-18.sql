...
left join table3 t3 on t1.List_name=t3.List_name_ch and t1.report_date=t3.business_date
left join table4 t4 on t1.List_name=t4.List_name_ch and t1.report_date=t4.business_date
left join table5 t5 on t1.List_name=t5.List_name_ch and t1.report_date=t5.business_date
);
...
t1.Total_Blocks
from table2 t2
left join  table1 t1 on t1.List_name=t2.List_name_ch and t1.report_date=t2.business_date
where t2.business_date<date_add(CURDATE(),interval 1 day) and t2.List_name_ch is not null
);



DROP TEMPORARY TABLE IF EXISTS table8;
CREATE TEMPORARY TABLE IF NOT EXISTS table8  
AS (
...
CREATE TEMPORARY TABLE IF NOT EXISTS table9  
AS (
...
CREATE TEMPORARY TABLE IF NOT EXISTS table10  
AS (
...
DROP TEMPORARY TABLE IF EXISTS table11;
CREATE TEMPORARY TABLE IF NOT EXISTS table11  
AS (
select * from (
select * from table7
union 
