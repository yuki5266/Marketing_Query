...
Audience 1: Withdrawn customers with the eligible withdrawn reasons. since 2015-01-01, 
    ...
    @valuation_date =curdate();
	 ...
	 set
		...
		@end_date= curdate();
			...
			Create TEMPORARY table if not exists raw1 (index (lms_customer_id, received_time))
      as(
			select  distinct
      @valuation_date,
			@channel,
      ...
      and la.isreturning = 0 
			...
			and la.lms_customer_id not in(select la2.lms_customer_id from reporting.leads_accepted la2 where la2.application_status='pending' or la2.origination_time is not null )
      and la.emailaddress not in (select distinct lacf.emailaddress from reporting_cf.leads_accepted lacf where lacf.emailaddress is not null)
						);

	-- SELECT * FROM raw1;

			Drop TEMPORARY table if exists raw2;
			Create TEMPORARY table if not exists raw2  (index (lms_customer_id, last_received_time)) 
      as(
			select  
			r1.lms_customer_id,
			max(r1.received_time) as last_received_time,
			r1.lms_code
			from raw1 r1
...
--  select * from raw2;
  ...
  Drop TEMPORARY table if exists raw3;
			Create TEMPORARY table if not exists raw3  (index (lms_customer_id, last_received_time)) 
      as( select r1.*,r2.last_received_time from raw1 r1 inner join raw2 r2 on r1.lms_code=r2.lms_code and r1.lms_customer_id=r2.lms_customer_id and r1.received_time=r2.last_received_time);
...
from reporting.campaign_history where list_module='WA' and date(list_generation_time)=curdate();
